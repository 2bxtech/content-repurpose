...
@pytest.fixture
async def websocket_client(authenticated_client: httpx.AsyncClient, websocket_url: str, test_workspace_id: str):
...
@pytest.fixture
async def mock_transformation_task(authenticated_client: httpx.AsyncClient, test_workspace_id: str):
    ...
    transform_response = await authenticated_client.post("/api/transformations", json=transformation_data)
    if transform_response.status_code != 201:
        pytest.fail(f"Failed to create transformation: {transform_response.status_code}")
    ...
    # Cleanup
    try:
        await authenticated_client.delete(f"/api/transformations/{transformation['id']}")
...
class WebSocketTestHelper:
    ...
    async def send_and_wait_for_response(
        ...
        while True:
            if asyncio.get_event_loop().time() - start_time > timeout:
                raise asyncio.TimeoutError(f"No response of type '{expected_type}' received within {timeout}s")
    ...
    async def wait_for_message_type(
        ...
        while True:
            if asyncio.get_event_loop().time() - start_time > timeout:
                raise asyncio.TimeoutError(f"No message of type '{message_type}' received within {timeout}s")
