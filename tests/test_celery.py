...
class TestCeleryIntegration:
    ...
    @pytest.mark.celery
    async def test_worker_status_endpoint(self, authenticated_client: httpx.AsyncClient):
    ...
    async def test_transformation_status_tracking(
        ...
        final_status = await wait_for_task(authenticated_client, transformation_id, timeout=10)
        ...
        if final_status["database_status"] == "completed":
            ...
            response = await authenticated_client.get(f"/api/transformations/{transformation_id}")
            assert response.status_code == 200
    ...
    async def test_concurrent_task_processing(
        ...
        for i in range(num_tasks):
            task_data = {
                **transformation_data,
                "parameters": {
                    **transformation_data["parameters"],
                    "task_number": i
                }
            }
