...
class PerformanceRegressionDetector:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.baselines_file = (
            self.project_root / "testing" / "performance" / "baselines.json"
        )
    ...
    def _check_regression(self, result: PerformanceResult) -> Optional[RegressionAlert]:
        ...
        regression_percent = (
            (result.duration - baseline.baseline_duration) / baseline.baseline_duration
        ) * 100
    ...
    def _handle_regression(self, regression: RegressionAlert):
        ...
        if regression.severity == "critical":
            print("   🔥 CRITICAL: Consider investigating immediately!")
        elif regression.severity == "major":
            print("   ⚠️  MAJOR: Investigate before next release")
        else:
            print("   ℹ️  MINOR: Monitor trend")
    ...
    def _update_baseline(self, result: PerformanceResult):
        ...
        recent_results = self._get_recent_results(
            result.test_name, result.operation, days=7
        )
    ...
    def _get_recent_results(
        self, test_name: str, operation: str, days: int = 7
                ...
                try:
                    ...
                    for result_data in daily_results:
                        if (
                            ...
                            and result_data["operation"] == operation
                            and result_data["timestamp"] >= cutoff_time
                        ):
    ...
    def generate_performance_report(self, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            "trends": {
                ...
                for key, durations in trends.items()
                if len(durations) > 1
            },
