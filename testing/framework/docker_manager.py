...
class DockerTestManager:
    ...
    def create_test_compose_profile(
        self, services: List[str] = None, test_name: str = "default"
    ) -> Path:
        """
        ...
        for service_name, service_config in compose_config.get("services", {}).items():
            ...
            if "container_name" in service_config:
                service_config["container_name"] = (
                    f"{service_config['container_name']}-{test_name}"
                )
    ...
    def wait_for_services_health(self, services: List[str], timeout: int = 300) -> bool:
        """
        ...
        try:
            ...
            if "connection refused" in logs.lower():
                diagnosis["issues"].append(
                    "Connection refused - service may not be fully started"
                )
                diagnosis["suggestions"].append(
                    "Wait longer for service initialization"
                )
            ...
            diagnosis["suggestions"].append(
                "Check if service is defined in docker-compose.test.yml"
            )
        except Exception as e:
    ...
    def stop_services(self, services: List[str] = None, remove_volumes: bool = True):
        ...
        try:
            cmd = ["docker-compose", "-f", str(self.test_compose_file), "down"]
    ...
    def cleanup_test_artifacts(self):
        ...
        patterns = ["docker-compose.test.*.yml", "testing/diagnostic_report.json"]
    ...
    @contextmanager
    def test_services(self, services: List[str] = None, auto_cleanup: bool = True):
        """
