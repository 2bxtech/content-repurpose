...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info(
        "Starting Content Repurposing Tool API v2.0.0 with Phase 8 Security & Monitoring"
    )
    ...
    try:
        ...
        if redis_service.is_connected():
            ...
            # Skip WebSocket Redis listener for now - this was likely causing the hang
            # await manager.start_redis_listener()
            logger.info("WebSocket Redis listener start skipped for debugging")
        else:
    ...
    try:
        ...
        logger.warning(f"Database initialization failed: {str(e)} - continuing with in-memory storage")
        if settings.ENVIRONMENT == "production":
            raise
...
# app.add_middleware(RateLimitMiddleware)  # Rate limiting - temporarily disabled for debugging
