...
class SecretManager:
    ...
    async def store_secret(
        ...
        try:
            ...
            # Create metadata
            rotation_days = rotation_policy_days or self.default_rotation_policies.get(secret_type)
            expires_at = None
            if rotation_days:
    ...
    async def rotate_secret(
        ...
        try:
            ...
            backup_file = self.secrets_dir / f"{secret_id}_backup_{int(datetime.now().timestamp())}.enc"
    ...
    def get_secrets_requiring_rotation(self, days_ahead: int = 7) -> List[SecretMetadata]:
    ...
    async def get_secret_analytics(self) -> Dict[str, Any]:
        ...
        active_secrets = len([m for m in self.metadata.values() if m.status == SecretStatus.ACTIVE])
        expired_secrets = len([m for m in self.metadata.values() if m.status == SecretStatus.EXPIRED])
        compromised_secrets = len([m for m in self.metadata.values() if m.status == SecretStatus.COMPROMISED])
        ...
        recent_accesses = [
            ...
            if datetime.fromisoformat(log.accessed_at) > datetime.now() - timedelta(hours=24)
        ]
        ...
        return {
            ...
            "failed_accesses_24h": len([log for log in recent_accesses if not log.success])
        }
...
class ConfigurationManager:
    ...
    async def update_secret_config(self, key: str, value: str, secret_type: SecretType) -> bool:
        """Update a configuration secret"""
        try:
            # Store new secret
            secret_id = await self.secret_manager.store_secret(
                name=key,
                value=value,
                secret_type=secret_type,
