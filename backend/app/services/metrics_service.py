...
class MetricsCollector:
            ...
            # For histograms and timers, accumulate values
                ...
                if existing_data:
                    ...
                    if isinstance(existing_metric['value'], list):
                        ...
                        existing_metric['value'] = [existing_metric['value'], metric.value]
    ...
    async def _trigger_alert(self, alert: Alert):
        ...
        await audit_service.log_event(
            event_type=AuditEventType.SYSTEM_WARNING if alert.severity == AlertSeverity.WARNING else AuditEventType.SYSTEM_ERROR,
            level=AuditLevel.WARNING if alert.severity == AlertSeverity.WARNING else AuditLevel.ERROR,
            details={
    ...
    async def get_metrics_summary(self, period_hours: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            while current_time <= end_time:
                ...
                keys = redis_service.redis_client.keys(pattern) if redis_service.redis_client else []
                for key in keys:
    ...
    def _aggregate_metrics_data(self, metrics_data: List[Dict[str, Any]]) -> Dict[str, Any]:
...
class PerformanceMonitor:
    ...
    async def get_memory_usage(self):
        ...
        try:
            ...
            return {"percent": memory.percent, "available_gb": memory.available / (1024**3)}
        except:
        ...
        try:
            ...
            if "api_response_time_ms" in summary.get("timers", {}):
                ...
                if api_times["p95"] > 2000:  # 2 second P95
                    await self.metrics_collector.record_metric(
                        "api_response_time_p95_violation",
                        1,
                        MetricType.COUNTER
                    )
    ...
    async def record_ai_request(self, provider: str, model: str, duration_ms: float, cost: float, success: bool):
    ...
    async def record_transformation_event(self, event_type: str, duration_ms: Optional[float] = None):
    ...
    async def record_user_activity(self, activity_type: str, user_count: Optional[int] = None):
    ...
    async def get_performance_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            # Get metrics summary
            metrics_summary = await self.metrics_collector.get_metrics_summary(period_hours=1)
    ...
    async def _calculate_kpis(self, metrics_summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            kpis["error_rate_percent"] = (error_requests / total_requests * 100) if total_requests > 0 else 0
