...
from .base import (
    BaseAIProvider, AIProviderType, AIResponse, AIUsageMetrics,
    AIProviderError, RateLimitError, QuotaExceededError, AIProviderStatus
)
...
class UsageTracker:
    ...
    costs_per_hour: deque = field(default_factory=lambda: deque(maxlen=3600))  # Store more cost entries
...
class AIProviderManager:
    ...
    def _initialize_providers(self):
        ...
        if settings.CLAUDE_API_KEY:
            self.provider_configs["anthropic"] = ProviderConfig(
                ...
                preferred_models=["claude-3-5-sonnet-20241022", "claude-3-sonnet-20240229"],
                fallback_models=["claude-3-haiku-20240307"]
            )
            self.providers["anthropic"] = AnthropicProvider(api_key=settings.CLAUDE_API_KEY)
        ...
        **kwargs
        ...
        for provider_name in providers_to_try:
            try:
                ...
                await self._track_usage(provider_name, response, time.time() - start_time)
    ...
    def _get_provider_order(self, preferred_provider: Optional[str] = None) -> List[str]:
        ...
        if self.selection_strategy == ProviderSelectionStrategy.PRIMARY_FAILOVER:
            ...
            if available_providers:
                self.provider_rotation_index = (self.provider_rotation_index + 1) % len(available_providers)
                ordered = available_providers[self.provider_rotation_index:] + available_providers[:self.provider_rotation_index]
                return ordered
    ...
    def _select_model(self, provider_name: str, requested_model: Optional[str] = None) -> str:
    ...
    async def _track_usage(self, provider_name: str, response: AIResponse, processing_time: float):
        """Track usage for rate limiting and cost management"""
        tracker = self.usage_trackers[provider_name]
        current_time = time.time()
        ...
        for name, provider in self.providers.items():
            ...
            status[name] = {
                ...
                "usage": {
                    ...
                    "hourly_cost": sum(c[1] for c in tracker.costs_per_hour if c[0] > time.time() - 3600)
                },
                "performance": perf
            }
    ...
    def get_cost_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for provider_name, tracker in self.usage_trackers.items():
            provider_cost = sum(
                c[1] for c in tracker.costs_per_hour 
                if c[0] > cutoff_time
            )
            provider_requests = len([
                t for t in tracker.requests_per_minute 
                if t > cutoff_time
            ])
        ...
        summary["total"] = {
            "cost": total_cost,
            "requests": sum(p["requests"] for p in summary.values() if isinstance(p, dict) and "requests" in p),
            "period_hours": hours
        }
