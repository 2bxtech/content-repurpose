    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                print(
                    f"WebSocket notification sent: {message_type} for transformation {transformation_id}"
                )
            else:
...
def get_transformation_prompt(
    ...
    if transformation_type == TransformationType.BLOG_POST:
        prompt = (
            base_prompt + "Transform this content into a well-structured blog post. "
        )
        if "word_count" in parameters:
            prompt += (
                f"The target word count is around {parameters['word_count']} words. "
            )
        ...
        prompt = (
            base_prompt
            + f"Create social media content for {platform} based on this information. "
        )
        ...
        if "sections" in parameters:
            prompt += (
                f"Include the following sections: {', '.join(parameters['sections'])}. "
            )
