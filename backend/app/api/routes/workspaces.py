...
from app.models.workspace import (
    ...
    WorkspaceMember, WorkspaceMemberList, WorkspaceInvite,
    WorkspaceSwitch, WorkspaceUsage, WorkspaceStats
)
...
async def get_current_workspace_context(
    ...
    stmt = select(UserDB).where(UserDB.id == user_id).options(selectinload(UserDB.workspace))
...
async def create_workspace(
    ...
    stmt = select(WorkspaceDB).where(
        and_(
            WorkspaceDB.slug == workspace_data.slug,
            WorkspaceDB.deleted_at.is_(None)
        )
    )
    result = await db.execute(stmt)
...
async def switch_workspace(
    ...
    logger.info(f"User {current_user['email']} switched to workspace {user.workspace.slug}")
