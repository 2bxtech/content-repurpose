...
from app.models.auth import (
    UserCreate, User, Token, UserLogin, RefreshTokenRequest, 
    LogoutRequest, PasswordChangeRequest, UserProfile, UserSession
)
...
async def register_user(
    ...
    if db:
        ...
        try:
            ...
            workspace = await workspace_service.create_default_workspace(db, None)  # Will set user_id after creation
            ...
            logger.info(f"New user registered: {user.email} (ID: {user_db.id}) with workspace: {workspace.slug}")
...
async def login_for_access_token(
    ...
    if db:
        ...
        if not user:
            logger.warning(f"Failed login attempt for: {form_data.username} from {auth_service._get_client_ip(request)}")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        logger.info(f"Successful login: {user.email} (ID: {user.id}) from {device_info.ip_address}")
...
async def logout(
    ...
    if logout_request.logout_all:
        ...
        logger.info(f"User logged out from all devices: {current_user['email']} (ID: {user_id})")
        ...
        logger.info(f"User logged out from device: {current_user['email']} (ID: {user_id})")
...
async def get_user_sessions(current_user: Annotated[dict, Depends(get_current_active_user)]):
    ...
    for session in sessions:
        session_list.append(UserSession(
...
async def change_password(
    ...
    is_strong, message = auth_service.validate_password_strength(password_request.new_password)
    if not is_strong:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=message
    ...
    if auth_service.verify_password(password_request.new_password, current_user["hashed_password"]):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
