...
async def test_websocket_connection():
    ...
    TEST_WORKSPACE = (
        "test-workspace-id"  # You'll need to replace with real workspace ID
    )
        ...
        async with websockets.connect(ws_url_with_auth) as websocket:
            ...
            try:
                ...
                if presence_data["type"] == "workspace_presence":
                    ...
                    print(
                        f"   Users online: {len(presence_data['data'].get('users', []))}"
                    )
                else:
            ...
            try:
                for i in range(5):
                    ...
                    print(
                        f"📬 Received: {msg_data['type']} - {msg_data.get('data', {})}"
                    )
            except asyncio.TimeoutError:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   Total connections: {stats['websocket_stats']['total_connections']}"
                )
                print(f"   Unique users: {stats['websocket_stats']['unique_users']}")
                print(
                    f"   Active workspaces: {stats['websocket_stats']['active_workspaces']}"
                )
            else:
