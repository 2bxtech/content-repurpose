...
def test_basic_functionality():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"✅ API responding: {data.get('name', 'Unknown')} v{data.get('version', 'Unknown')}")
            print(f"   Features: {len(data.get('features', []))} security features listed")
        else:
        ...
        if response.status_code == 201:
            ...
            print(f"   Created user: {user_data.get('email')} (ID: {user_data.get('id')})")
        else:
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                ...
                print(f"   User profile: {user_profile.get('email')} - {user_profile.get('active_sessions')} sessions")
            else:
