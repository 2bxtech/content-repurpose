...
class Phase5TestRunner:
    ...
    def start_services(self):
        ...
        try:
            redis_process = self.run_command(["redis-server", "--port", "6379"], background=True)
        ...
        try:
            celery_process = self.run_command([
                sys.executable, "-m", "celery", "-A", "app.core.celery_app", 
                "worker", "--loglevel=info"
            ], cwd=self.backend_dir, background=True)
